...
\usepackage{graphicx}%
\graphicspath{{Figures/}}%% all \includegraphics calls below resolve relative to this subfolder;
                          %% upload the Figures/ folder (not its contents into the project root)
\usepackage{multirow}%
...
%% LaTeX's default float-placement limits (max 2 floats at the top of a page, 1 at
%% the bottom, 3 total, and floats give up if less than 20% of a page is left for
%% text) are too conservative for a paper with 41 figures, several stacked back to
%% back -- with the defaults, floats queue up faster than they can be placed,
%% leaving pages mostly blank while a figure strands itself near the bottom of a
%% later page. Relaxing these lets LaTeX pack floats far more tightly instead.
...
Figure~\ref{fig:framework_proposed} shows the Digital Twin framework as originally proposed at the project-proposal stage, and Figure~\ref{fig:framework_updated} shows the as-built architecture reflecting the deviations listed in Table~\ref{tab1}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_framework_proposed.png}
\caption{Neuro-DT framework as originally proposed.}
\label{fig:framework_proposed}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_framework_updated.png}
\caption{As-built Neuro-DT framework architecture, reflecting the deviations summarized in Table~\ref{tab1}.}
\label{fig:framework_updated}
\end{figure}
...
Figures~\ref{fig:data_ingestion_job}-\ref{fig:final_manifest_sample} document these pipeline stages as run.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_data_ingestion_job.png}
\caption{Data-ingestion job (pipeline stage 1, manifest construction).}
\label{fig:data_ingestion_job}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_data_ingestion_output.png}
\caption{Sample output of the data-ingestion job, the joined dataframe linking scans to clinical records.}
\label{fig:data_ingestion_output}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_archive_extraction_job.png}
\caption{Archive-extraction job (pipeline stage 2).}
\label{fig:archive_extraction_job}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_dicom_validation.png}
\caption{DICOM integrity validation log (pipeline stage 3).}
\label{fig:dicom_validation}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_3d_volume_validation.png}
\caption{Reconstructed 3D MRI slices used to validate the DICOM-to-tensor pipeline end to end (pipeline stage 4).}
\label{fig:3d_volume_validation}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_final_manifest_sample.png}
\caption{Sample of the final joined manifest (pipeline stage 5).}
\label{fig:final_manifest_sample}
\end{figure}
...
Figures~\ref{fig:azure_devbox_instance} and~\ref{fig:azure_devbox_cluster} show the provisioned specifications for both.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_azure_devbox_instance.png}
\caption{Azure Machine Learning DevBox compute instance specification.}
\label{fig:azure_devbox_instance}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_azure_devbox_cluster.png}
\caption{Azure Machine Learning Workhorse compute cluster specification.}
\label{fig:azure_devbox_cluster}
\end{figure}
...
Figure~\ref{fig:cpu_fold_recovery_log} shows the checkpoint log across all five fold attempts, confirming Fold 4 as the only one to complete.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_cpu_fold_recovery_log.png}
\caption{Fold checkpoint log showing only Fold 4 completing all 15 epochs while the remaining folds triggered early stopping.}
\label{fig:cpu_fold_recovery_log}
\end{figure}
 The full training job (all five fold attempts, run as a single Azure ML Command Job, \texttt{BDT-Hybrid-Model-Training-CleanData}) ran for \textbf{1 day, 10 hours} of compute time, the great majority of which was consumed by Fold 4's complete 15-epoch run; the four early-stopped folds each contributed comparatively little wall-clock time by comparison.
...
The model produced zero misclassifications between the CN and Dementia classes, the two diagnostically furthest-apart categories, across the entire validation set (Figure~\ref{fig:cpu_confusion_matrix}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_cpu_confusion_matrix.png}
\caption{Fold 4 confusion matrix (CPU-trained model).}
\label{fig:cpu_confusion_matrix}
\end{figure}
...
Figures~\ref{fig:cpu_ablation_horizontal_chart}-\ref{fig:cpu_ablation_training_logs} show this comparison from the CPU-feasible ablation run in full, including the per-class breakdown and the underlying training logs for the A0/A1 variants.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_cpu_ablation_horizontal_chart.png}
\caption{CPU-feasible ablation results, horizontal comparison chart.}
\label{fig:cpu_ablation_horizontal_chart}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_cpu_ablation_perclass.png}
\caption{CPU-feasible ablation results, per-class AUC.}
\label{fig:cpu_ablation_perclass}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_cpu_ablation_training_logs.png}
\caption{Training curves and classification reports for the A0 and A1 CPU-feasible variants.}
\label{fig:cpu_ablation_training_logs}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_cpu_ablation_results.png}
\caption{CPU-feasible ablation results, bar chart.}
\label{fig:cpu_ablation_results}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_cpu_shap.png}
\caption{SHAP tabular feature attribution (CPU-trained model).}
\label{fig:cpu_shap}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_cpu_gradcam_057_1373.png}
\caption{Grad-CAM activation overlay, Patient 057\_S\_1373 (Dementia), CPU-trained model.}
\label{fig:cpu_gradcam}
\end{figure}
...
Figure~\ref{fig:cpu_markov_heatmap} shows the corrected CPU-side transition-matrix heatmap, and Figures~\ref{fig:cpu_whatif_016_1149}-\ref{fig:cpu_whatif_numbers_016_1149} show the CPU-trained model's what-if trajectories and summary numbers for Patient 016\_S\_1149, the same patient re-examined under the GPU-trained model in Section 5.2.11.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_cpu_markov_heatmap.png}
\caption{Markov transition matrix heatmap, full cohort, CPU run.}
\label{fig:cpu_markov_heatmap}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_cpu_whatif_016_1149.png}
\caption{What-if simulation trajectories for Patient 016\_S\_1149 under the CPU-trained model.}
\label{fig:cpu_whatif_016_1149}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_cpu_whatif_numbers_016_1149.png}
\caption{What-if simulation summary numbers for Patient 016\_S\_1149 under the CPU-trained model.}
\label{fig:cpu_whatif_numbers_016_1149}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_gpu_detection.png}
\caption{GPU detection and specification confirmation at the start of Experiment 2.}
\label{fig:gpu_detection}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.45\textwidth]{fig_gpu_5fold_training_log.png}
\caption{5-fold training log showing all folds completing under the revised GPU training configuration.}
\label{fig:gpu_5fold_training_log}
\end{figure}
...
Figure~\ref{fig:gpu_baseline_training_logs} shows the per-variant training logs for the classical baselines (B1-B4) from this run, and Figure~\ref{fig:gpu_ablation_perclass_delta} shows each variant's per-class AUC change relative to the full multimodal model.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.38\textwidth]{fig_gpu_baseline_training_logs.png}
\caption{Per-variant classical-baseline (B1-B4) training logs from the full GPU ablation study.}
\label{fig:gpu_baseline_training_logs}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_gpu_ablation_perclass_delta.png}
\caption{Per-class AUC change relative to the full multimodal model for each ablation variant.}
\label{fig:gpu_ablation_perclass_delta}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_gpu_ablation_11variant.png}
\caption{Ablation study, AUC comparison across all 11 variants.}
\label{fig:gpu_ablation_11variant}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_gpu_ablation_perclass.png}
\caption{Ablation study, per-class AUC grouped chart.}
\label{fig:gpu_ablation_perclass}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_gpu_confusion_matrix.png}
\caption{Fold 4 confusion matrix, GPU-trained model.}
\label{fig:gpu_confusion_matrix}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_gpu_shap.png}
\caption{SHAP tabular feature attribution, GPU-trained model.}
\label{fig:gpu_shap}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_gpu_gradcam_057_1373.png}
\caption{Grad-CAM activation overlay, Patient 057\_S\_1373 (Dementia), GPU-trained model.}
\label{fig:gpu_gradcam}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_gpu_markov_heatmap.png}
\caption{Markov transition matrix heatmap, full cohort, GPU run.}
\label{fig:gpu_markov_heatmap}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_gpu_whatif_016_1149.png}
\caption{What-if simulation for Patient 016\_S\_1149: baseline, APOE4, Lecanemab, and Donanemab trajectories under the GPU-trained model.}
\label{fig:gpu_whatif}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_age_sensitivity_016_1149.png}
\caption{Age-sensitivity analysis, Patient 016\_S\_1149.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_population_subgroup.png}
\caption{Population subgroup trajectory comparison across CN, MCI, and Dementia.}
\label{fig:population_subgroup}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig_early_late_intervention_128_0167.png}
\caption{Early vs. late intervention timing, Patient 128\_S\_0167.}
\label{fig:early_late_intervention}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_dashboard_prediction.png}
\caption{Deployed dashboard, prediction output for the held-out verification patient.}
\label{fig:dashboard_prediction}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_dashboard_about.png}
\caption{Deployed dashboard, ``About'' / model-info panel.}
\label{fig:dashboard_about}
\end{figure}


Beyond the prediction and model-info views already discussed, the dashboard exposes several further panels not separately narrated above: a 5-year state-trajectory view, a dementia risk-factor breakdown, a Grad-CAM demo on a different patient, and an AI-generated lifestyle-recommendation panel (Figures~\ref{fig:dashboard_trajectories_tab}-\ref{fig:dashboard_lifestyle}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_dashboard_trajectories_tab.png}
\caption{Deployed dashboard, 5-year state-trajectories tab.}
\label{fig:dashboard_trajectories_tab}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_dashboard_risk_factor.png}
\caption{Deployed dashboard, 5-year dementia risk-factor chart.}
\label{fig:dashboard_risk_factor}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_dashboard_gradcam_demo.png}
\caption{Deployed dashboard, Grad-CAM demonstration on Patient 023\_S\_0887.}
\label{fig:dashboard_gradcam_demo}
\end{figure}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{fig_dashboard_lifestyle.png}
