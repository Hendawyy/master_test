...
<div class="metric-row">
  <div class="m-box"><div class="m-val">0.951</div><div class="m-lbl">Fold 4 AUC</div></div>
  <div class="m-box"><div class="m-val">85%</div><div class="m-lbl">Accuracy</div></div>
  <div class="m-box"><div class="m-val">0.85</div><div class="m-lbl">Macro F1</div></div>
  <div class="m-box"><div class="m-val">1,549</div><div class="m-lbl">Training Scans</div></div>
</div>
<p style="text-align:center;color:#64748b;font-size:.8rem;margin-top:.4rem">
  Fold 4 is this dashboard's deployed checkpoint. Honest 5-fold cross-validated estimate:
  <b>0.871 &plusmn; 0.046 AUC</b> (see thesis methodology) -- not just the single best fold.
</p>
""", unsafe_allow_html=True)
...
**Parameters:** 24M | **Training:** AdamW lr=1e-4, cosine LR, batch=4, 5-fold CV
    ...
    with c2:
        ...
        results_df = pd.DataFrame({
            ...
            'Precision': ['88%', '85%', '82%', '85%'],
            'Recall':    ['94%', '84%', '78%', '85%'],
            'F1':        ['0.91', '0.85', '0.80', '0.85'],
            'AUC':       ['0.982', '0.958', '0.913', '0.951'],
        })
    ...
    lit_df = pd.DataFrame({
        ...
        'AUC / Accuracy': [
            ...
            '1.8% MAE reduction', '**0.951 AUC · 85% acc**',
        ],
        'MRI imaging':    ['✓','✓','✓','✗','✓','✓','✗','✗','✓'],
